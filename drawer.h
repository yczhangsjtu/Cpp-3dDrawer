...
Object newObject(ObjectType type, GLfloat R, GLfloat G, GLfloat B,
				 GLfloat specular, GLfloat shrininess, GLfloat r = 0)
{
	Object object;
	object.type = type;
	object.color[0] = R;
...
void useMaterial(GLfloat R,GLfloat G,GLfloat B,GLfloat specular,GLfloat shrininess)
{
	...
	glMaterialf(GL_FRONT,GL_SHININESS,material.shrininess);
}
...
void drawLine(const GLfloat (*p)[3], const GLfloat *color,
			  GLfloat shrininess = 42.0,GLfloat specular = 0.4)
{
				  ...
				  int id = 0, GLenum mode = GL_RENDER)
{
	...
	if(d2/r/r<0.0001)
	{
		drawCircle(p,r,color,shrininess,specular,id,mode);
	}
	else
	{
		...
		for(int i = 0; i < n; i++)
			for(int j = 0; j < n; j++)
			{
				...
				drawSquare(p,color,shrininess,specular);
			}
	}
}
			  ...
			  int id = 0, GLenum mode = GL_RENDER)
{
	...
	drawCircle(p1,r,color,shrininess,specular);
	drawCircle(p2,r,color,shrininess,specular);
	...
	for(int i = 0; i < n; i++)
	{
		...
		{{a11*r*ct[0]+a21*r*st[0]+a31*l+p[0][0],
		 ...
		 drawSquare(points,color,shrininess,specular,id,mode);
	}
}
	...
	for(int i = 0; i < n; i++)
	{
		...
		{{a31*l+p[0][0],
		 ...
		 {a11*r*ct[0]+a21*r*st[0]+p[0][0],
		  a21*r*ct[0]+a22*r*st[0]+p[0][1],
		  a31*r*ct[0]+a32*r*st[0]+p[0][2]}};
		 drawTriangle(points,color,shrininess,specular,id,mode);
	}
}
...
void drawBoldLine(const GLfloat (*p)[3], const GLfloat *color,
			  ...
			  int id = 0, GLenum mode = GL_RENDER)
{
	drawCylinder(p,0.1,color,shrininess,specular,id,mode);
}
...
void drawObject(const Object &object)
{
	...
	{
		drawSquare(object.points,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Line)
	{
		drawLine(object.points,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Triangle)
	{
		drawTriangle(object.points,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Ball)
	{
		drawBall(object.points[0],object.radius,object.color,object.shrininess,object.specular);
	}
	else if(object.type == BallPart)
	{
		drawBallPart(object.points,object.radius,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Circle)
	{
		drawCircle(object.points,object.radius,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Tetrahedron)
	{
		drawTetrahedron(object.points,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Cylinder)
	{
		drawCylinder(object.points,object.radius,object.color,object.shrininess,object.specular);
	}
	else if(object.type == BoldLine)
	{
		drawBoldLine(object.points,object.color,object.shrininess,object.specular);
	}
	else if(object.type == Cone)
	{
		drawCone(object.points,object.radius,object.color,object.shrininess,object.specular);
	}
}
...
bool intepret(const std::string &str, Object *object)
{
	...
	if(type=="Square")
	{
		...
		if(!is.eof()) is >> (object->specular) >> (object->shrininess);
		else
		{
			object->specular = specular;
			object->shrininess = shrininess;
...
std::string express(const Object & object)
{
	...
	os << object.specular << " " << object.shrininess;
	return os.str();
}
